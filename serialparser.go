...
func main() {
        ...
        responsedoorstatus := []byte{0x7E,0x9e,0x00,0x00,0x00,0x01,0x67,0x36,0x00,0x00,0x7E,0x0d,0x0a}
        passengercounterdata := []byte{0x7E, 0xA4, 0x00, 0x00, 0x00, 0x01, 0x67, 0x38, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x03, 0x7E,0x0d,0x0a}
        sendhandshake      := []byte{0x7E,0x9c,0x00,0x00,0x00,0x01,0x67,0x34,0x7E,0x0d,0x0a}
        cr := []byte{0x0d}
        lf := []byte{0x0a}
        buff := make([]byte, 1)
        counter :=0;
                                ...
                                if (string(buff[:n]) == string(lf)) {
                                        ...
                                        for _, x := range parse {
                                                if x == handshake[pointer] {
                                                        pointer = pointer + 1
                                                        if pointer == 9 {
                                                                sendhandshake[3] = byte(counter)
                                                                _, err := port.Write(handshake)

                                                                if err != nil {
                                                                        log.Fatalf("port.Write: %v", err)
                                                                } else {
                                                                        log.Printf("TX:\n%v", hex.Dump(sendhandshake))
                                                                }
                                                                counter = counter + 1
                                                                break
                                                        }
                                        ...
                                        for _, x := range parse {
                                                if x == querydoorstatus[pointer] {
                                                        ...
                                                        if pointer == 9 {
                                                                ...
                                                                passengercounterdata[3]=byte(counter)
